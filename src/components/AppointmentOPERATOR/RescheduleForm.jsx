...
const RescheduleFormAppointment = ({ props }) => {
    const { setSelectedDate, hoursAvailable, onSubmit, register, setCita, errors, isModalOpen } = useAppointment_ReScheduler();    // Custom hook
    useEffect(() => {
        setCita(props)
    }, [setSelectedDate]);
    ...
    return (
        <>
            {isModalOpen ? <BasicModal title={'Cita re agendada con éxito'} description={'La cita ha sido re-agendada con exito, avisele al paciente que revise su correo electronico'} /> : ''}
